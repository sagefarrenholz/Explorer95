...
// Send request for file directory
...
class List_Item extends React.Component{
  ...
  render() {
    ...
    if(source.isDirectory === true) {
      ...
      img = <div class="dir_icon"/>;
      return <div className="item directory">{img}<a href={_href}>{label}</a></div>;
    } else {
      // TODO Ad indivualized icons
      img = <div class="file_icon"/>;
      return <div className="item file">{img}<a href={label}>{label}</a></div> ;
    }
  }
}
  ...
  render() {
    ...
    if (file_dir !== null) {
      for (const entry in this.props.dir) {
        list.push(<List_Item key={entry} label={entry} source={this.props.dir[entry]}/>);
      }
    }
    return (
      <div>
